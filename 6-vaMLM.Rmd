...
normally ditributed.
model with gaussian dsitribution
```{r cs distribution}
ggplot(va, aes(x = va, fill = condition))+
...
MAXIMAL MODEL: va ~ condition*age + resID + (1|resID) + (1|id) 

MINIMAL MODEL: va ~ condition*age + (1|resID) + (1|id)
```{r va gausian mlm}
...
# gauMax <- lmer(va ~ condition*age.s + resID + #FIXED
#                  (1|resID) + (1|id), #RANDOM
#          data = d,
...
# bobyqa - converged, no evidence for interaction
# Nelder_Mead -
# nloptwrap - 
# gauTwo <- lmer(va ~ condition*age.s + resID + #FIXED
#                  (1|id), #RANDOM
#          data = d,
...
# Nelder_Mead - 
# nloptwrap - 
gauThree <- lmer(va ~ condition + age.s + resID + #FIXED
                 (1|id), #RANDOM
         data = d, 
         control = lmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 10000000))
         )
...
modelsummary(list("Gaussian" = gauFinal), 
             stars = T, 
             title = "VA MLM output",
             ...
             output = "vaDistTable.html" #statistics in separate columns
