...
Don't load these too early as they muck about with tidy verse
```{r mlm packages}
library(lme4)
library(lmerTest)
require(car)
...
confint(simVaGammaCondition, method = "Wald")

```

let's try another distribution. see if that works
```{r va inverse gausian}
vaInv <- glmer(VA.t ~ condition + (1|Participant.Private.ID),
                 family = inverse.gaussian(link = "identity"),
                   data = d, 
                 glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 10000000)))

summary(vaInv)
Confint(vaInv, method = "Wald")

anova(vaGammaFinal, vaInv)
```

```{r va final mod}

vaFinal <- vaGammaFinal
summary(vaFinal)
vaConfint <- Confint(vaFinal)
vaConfint[1,] <- vaConfint[1,]-1
